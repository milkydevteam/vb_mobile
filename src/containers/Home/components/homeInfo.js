...
export default class HomeInfo extends PureComponent<Props> {
  ...
  setLang = () => {
    const current = langService.getCurrentLocale();
    if (current === 'vi') {
      langService.setLocale('en');
      return;
    }
    langService.setLocale('vi');
  };
  render() {
    return (
      <View style={style.container}>
        ...
        <View style={style.endView}>
          <TouchableOpacity style={style.btn} onPress={this.setLang}>
            <Text style={style.textColor}>
              {langService.getCurrentLocale().toUpperCase()}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
